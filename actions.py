...
class BookRoomInfo(FormAction):
    ...
    def submit(
        ...
        dispatcher.utter_message(template="utter_submit", number=tracker.get_slot('number'),
                                 room_type=tracker.get_slot('room_types'))
        return []

    def slot_mappings(self) -> Dict[Text, Union[Dict, List[Dict]]]:

        return {
            "number": self.from_entity(entity="number", intent='number_of_rooms'),
            "room_types": self.from_entity(entity="room_types", intent="room_type")
        }
...
class ResetSlots(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        return [SlotSet("number", None), SlotSet("room_types", None)]
...
class ActionCheckInTime(Action):
    ...
    def name(self):
        return "action_checkin_timing"

    def run(self, dispatcher, tracker, domain):
        dispatcher.utter_template("utter_checkin_timing", tracker)
        repeat(tracker, dispatcher)        
        return [UserUtteranceReverted()]
...
class ActionCheckOutTime(Action):
    ...
    def name(self):
        return "action_checkout_timing"

    def run(self, dispatcher, tracker, domain):
        dispatcher.utter_template("utter_checkout_timing", tracker)
        repeat(tracker, dispatcher)
        return [UserUtteranceReverted()]
...
class ActionHaveRestaurant(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        dispatcher.utter_template("utter_check_restaurant", tracker)
        repeat(tracker, dispatcher)
        return [UserUtteranceReverted()]
...
class ActionBreakfastTime(Action):
    ...
    def name(self):
        return "action_time_breakfast"

    def run(self, dispatcher, tracker, domain):
        dispatcher.utter_template("utter_time_breakfast", tracker)
        repeat(tracker, dispatcher)
        return [UserUtteranceReverted()]
